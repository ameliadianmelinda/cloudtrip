...
@section('content')
<h2 class="fw-bold mb-4">Tambah Pesawat</h2>
    ...
    <form action="{{ route('pesawat.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('kode_pesawat') is-invalid @enderror" 
                name="kode_pesawat" value="{{ old('kode_pesawat') }}">
            @error('kode_pesawat')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control @error('tipe_pesawat') is-invalid @enderror" 
                name="tipe_pesawat" value="{{ old('tipe_pesawat') }}">
            @error('tipe_pesawat')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select class="form-control @error('maskapai_id') is-invalid @enderror" name="maskapai_id">
                ...
                @foreach ($maskapai as $item)
                    <option value="{{ $item->maskapai_id }}" {{ old('maskapai_id') == $item->maskapai_id ? 'selected' : '' }}>
                        {{ $item->nama_maskapai }}
                    </option>
                @endforeach
            </select>
